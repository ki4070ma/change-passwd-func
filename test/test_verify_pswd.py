...
class TestVerifyPasswd(object):
    class TestVerifyPasswdLength(object):
        ...
        @pytest.mark.parametrize(
            "length", [MIN_VALID_LENGTH, MIN_VALID_LENGTH + 1, MAX_VALID_LENGTH]
        )
    ...
    class TestVerifyPassowdContinuousSameChar(object):
        # TestIdx: 11, 12
        @pytest.mark.parametrize(
            "continous_num", [MAX_CHAR_CONTINUOUS_NUM - 1, MAX_CHAR_CONTINUOUS_NUM]
        )
    ...
    class TestVerifyPasswdNumericLimitation(object):
        ...
        def test_verify_pswd_num_less_than_half_of_length_odd(self, length):
            assert verify_pswd(
                pswd_generator(pswd_base="!12", length=length, incl_sp_char=False)
            )
        ...
        def test_verify_pswd_num_more_than_half_of_length_odd(self, length):
            assert not verify_pswd(
                pswd_generator(pswd_base="!123", length=length, incl_sp_char=False)
            )
        ...
        def test_verify_pswd_num_less_than_half_of_length_even(self, length):
            assert verify_pswd(
                pswd_generator(pswd_base="!1", length=length, incl_sp_char=False)
            )
